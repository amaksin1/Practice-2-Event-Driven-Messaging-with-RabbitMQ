...
When `POST /orders` is called on the Order Service, the service generates a UUID v4 `correlationId`, enriches the incoming payload with `orderId` and `timestamp`, stores the enriched order in memory, and publishes the order event to the RabbitMQ fanout exchange `orders.exchange`. The message body is JSON and the same `correlationId` is also placed into AMQP message headers so that the order can be traced end-to-end across services. Because the exchange is fanout, RabbitMQ delivers a copy of the message to all three consumer queues: `payments.queue`, `inventory.queue`, and `notifications.queue`.

Each consumer processes the message with manual acknowledgements (`prefetch(1)` and explicit `ack`). If processing succeeds, the consumer acknowledges the original message and publishes a success result event to its own results exchange (`results.payment`, `results.inventory`, or `results.notification`) while preserving the same `correlationId` in headers. If processing fails, the consumer nacks the message with `requeue=false`, which routes it through the retry topology (DLX -> per-service retry queue with TTL -> return exchange -> original queue). After the message has been retried up to `MAX_RETRIES`, the consumer breaks the retry loop by publishing the original message to `orders.dlq.exchange` and acknowledging it so it is removed from the consumer queue. The Notification Service additionally performs an idempotency check based on the `correlationId` to ensure duplicate deliveries do not create duplicate log entries.
...
2. **EAI Pattern mapping table** — identify where each pattern is applied and explain it in your own words:
    ...
    |---|---|---|
    | Publish-Subscribe Channel | RabbitMQ fanout exchange `orders.exchange` bound to `payments.queue`, `inventory.queue`, `notifications.queue` | One published order event is broadcast by the broker so multiple independent services receive and process the same event in parallel without the publisher knowing the subscribers. |
    | Dead Letter Channel | Retry/DLQ topology using `orders.retry.exchange`, `*.retry.queue`, and final DLQ `orders.dlq.exchange` -> `orders.dlq` | Failed messages are not lost; they are rerouted to a dedicated channel/queue for retries and, after exhausting attempts, to a DLQ for later inspection. |
    | Correlation Identifier | `correlationId` UUID generated in Order Service, propagated in AMQP headers by all services and result events | A single identifier is carried with every message so events from different services can be tied back to the same original order across asynchronous flows. |
    | Guaranteed Delivery | Manual acknowledgements (`ack` on success, `nack(..., requeue=false)` on failure) and retry with TTL | Messages are only removed from queues when a consumer explicitly acknowledges them; failures trigger a controlled retry path, reducing the chance of silent loss. |
    | Idempotent Receiver | Notification Service uses `/data/processed-ids.json` and checks `correlationId` before writing to `/data/notification.log` | The receiver records processed correlation IDs so repeated delivery of the same event is detected and safely ignored, preventing duplicate side effects. |
